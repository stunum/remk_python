...
class ExaminationCreate(BaseModel):
    """检查创建模型"""
    examination_number: Optional[str] = PydanticField(
    ...
    examination_date: datetime = PydanticField(
        ..., description="检查日期时间（将自动拆分为日期和时间）")
    eye_side: Optional[str] = PydanticField(
        None, description="检查眼别：left/right/both")
    ...
    examination_findings: Optional[str] = PydanticField(
        None, description="检查所见")
    preliminary_diagnosis: Optional[str] = PydanticField(
        None, description="初步诊断")
    ...
    status: str = PydanticField(
        default="pending", description="检查状态：pending/in_progress/completed/cancelled")
...
class ExaminationUpdate(BaseModel):
    """检查更新模型"""
    examination_number: Optional[str] = PydanticField(
        None, min_length=1, max_length=50, description="检查编号")
    ...
    examination_time: Optional[time_type] = PydanticField(
        None, description="检查时间")
    eye_side: Optional[str] = PydanticField(
        None, description="检查眼别：left/right/both")
    chief_complaint: Optional[str] = PydanticField(None, description="主诉")
    present_illness: Optional[str] = PydanticField(None, description="现病史")
    ...
    status: Optional[str] = PydanticField(
        None, description="检查状态：pending/in_progress/completed/cancelled")
    notes: Optional[str] = PydanticField(None, description="备注")
    updated_by: Optional[int] = PydanticField(None, gt=0, description="更新人ID")
...
class ExaminationDeleteRequest(BaseModel):
    """检查批量删除请求模型"""
    examination_ids: List[int] = PydanticField(
        ..., min_length=1, description="要删除的检查ID列表")
...
async def create_examination(
    ...
    try:
        ...
        if not examination.examination_number:
            examination.examination_number = generate_examination_number(
                session)
        ...
        if isinstance(examination_datetime, datetime):
            ...
            log.info(
                f"拆分日期时间: 日期={examination_dict['examination_date']}, 时间={examination_dict['examination_time']}")
        else:
            # 如果是其他类型，直接使用
        ...
        log.info(
            f"成功创建检查记录: ID={db_examination.id}, 检查编号={db_examination.examination_number}")
        return success_response(
            data=ExaminationResponse.model_validate(
                db_examination).model_dump(),
            msg="检查记录创建成功"
        )
...
async def get_examinations(
    ...
    try:
        ...
        if examination_type_id:
            count_query = count_query.filter(
                Examination.examination_type_id == examination_type_id)
        ...
        if examination_number:
            count_query = count_query.filter(
                Examination.examination_number.ilike(f"%{examination_number}%"))
        ...
        for exam in examinations:
            ...
            if exam.examination_type_id:
                ...
                if exam_type:
                    exam_dict['examination_type'] = ExaminationTypeInfo.model_validate(
                        exam_type).model_dump()

            # 查询关联的医生
            if exam.doctor_id:
                doctor = session.query(User).filter(
                    User.id == exam.doctor_id).first()
                if doctor:
                    exam_dict['doctor'] = UserInfo.model_validate(
                        doctor).model_dump()

            # 查询关联的技师
            if exam.technician_id:
                technician = session.query(User).filter(
                    User.id == exam.technician_id).first()
                if technician:
...
@router.get("/today-examination/{patient_id}", response_model=ResponseModel, summary="查询患者当天是否有检查记录", dependencies=[Depends(get_current_user_info), Depends(require_permission('EXAMINATION_VIEW'))])
async def get_patient_today_examination(patient_id: int, session: Session = Depends(get_db)):
    try:
        today = date.today()
        # 构建基础查询 - 用于计算总数
        ...
        if not examination:
            log.warning(f"患者当天无检查记录")
            return error_response(msg="患者当天无检查记录", code=404)
        log.info(f"根据患者ID查询成功: {patient_id}")
        return success_response(
            data=ExaminationResponse.model_validate(examination).model_dump()
        )
    except Exception as e:
        log.error(f"根据患者ID查询失败: {str(e)}")
...
async def get_examination(
    ...
    try:
        ...
        if examination.doctor_id:
            doctor = session.query(User).filter(
                User.id == examination.doctor_id).first()
            if doctor:
                exam_dict['doctor'] = UserInfo.model_validate(
                    doctor).model_dump()
        ...
        if examination.technician_id:
            technician = session.query(User).filter(
                User.id == examination.technician_id).first()
            if technician:
                exam_dict['technician'] = UserInfo.model_validate(
                    technician).model_dump()
...
async def update_examination(
    ...
    try:
        ...
        if examination_update.examination_number and examination_update.examination_number != db_examination.examination_number:
            ...
            if existing:
                log.warning(
                    f"检查编号已存在: {examination_update.examination_number}")
