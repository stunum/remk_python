...
class SaveMultiImageRequest(BaseModel):
    ...
    examination_id: int = PydanticField(..., gt=0, description="检查ID")
    file_dir: str = PydanticField(..., description="文件目录路径")
    image_name: List[str] = PydanticField(..., min_length=1, description="图片文件名列表")
    eye_side: str = PydanticField(..., description="眼别：OD(右眼)/OS(左眼)")
    image_type: Optional[str] = PydanticField(None, description="图像类型")
    resolution: Optional[str] = PydanticField(None, description="分辨率")
    file_format: str = PydanticField(..., description="文件格式")
...
async def save_image_to_local(
    ...
    """
    try:
        log.info(f"保存单张图片: examination_id={request.examination_id}, image_name={request.image_name}")
        
        # 生成影像编号
        image_number = generate_image_number(request.examination_id)
        
        # 构建完整文件路径
        full_path = os.path.join(request.file_dir, request.image_name)
        ...
        # 转换为Base64（压缩）
        thumbnail_data = img_path_to_base64(full_path)
...
async def save_multi_image_to_local(
    ...
    """
    try:
        log.info(f"保存多张图片: examination_id={request.examination_id}, 图片数量={len(request.image_name)}")
        ...
        for img_name in request.image_name:
            ...
            if img_name.endswith("_G.jpg"):
                green_img = os.path.join(request.file_dir, img_name)
            elif img_name.endswith("_IR.jpg"):
                ir_img = os.path.join(request.file_dir, img_name)
            elif img_name.endswith("_R.jpg"):
                red_img = os.path.join(request.file_dir, img_name)
            elif img_name.endswith("_B.jpg"):
                blue_img = os.path.join(request.file_dir, img_name)
            
            # 构建完整文件路径
            full_path = os.path.join(request.file_dir, img_name)
        ...
        if ir_img and green_img:
            ...
            color_img_path = os.path.join(request.file_dir, color_img_name)
            ...
            try:
                ...
                # 转换彩色图像为Base64
                color_thumbnail_data = img_path_to_base64(save_path)
